...
process_survey_data <- function(sheet_name, country_name) {
  survey_data <- read_excel(paste0(ken_path,"ken_holpa_household_survey_clean.xlsx"), sheet = sheet_name)
...
##### Zimbabwe ----
# Define file path
ken_path <- "C:/Users/andreasanchez/OneDrive - CGIAR/Bioversity/AI/HOLPA/HOLPA_data/Kenya/kenya_data_clean/" #household form

ken_h_choices <- read_excel(paste0(ken_path,"ken_holpa_household_form_clean.xlsx"), sheet = "choices")%>%
  mutate(country= "kenya",name_new=NA)%>%
...
write.csv(ken_global_choices,"ken_global_choices.csv",row.names=FALSE)
...
ken_f_survey<- read.csv(paste0(ken_path,"ken/ken_fieldwork_format.csv"))
colnames(ken_f_survey) <- gsub("^X", "", colnames(ken_f_survey))
ken_f_survey<-ken_f_survey%>%
...
#Soil results 
## TO CHECK!!
ken_soil_results <- read_excel(paste0(ken_path,"ken_soil_results_250924.xlsx"), sheet = "Resultados suelos")

# Read all sheet names from ken_household survey
sheet_names <- excel_sheets(paste0(ken_path,"ken_holpa_household_survey_clean.xlsx"))
sheet_names

# Define country name and column to rename (adjust accordingly)
country_name <- "kenya"  # Replace with actual country name
...
walk(sheet_names, function(sheet) {
  df_name <- paste0("ken", sheet)  # Create dynamic name
  assign(df_name, process_survey_data(sheet, country_name), envir = .GlobalEnv)
})

ken_maintable <- `kenHolpa_global_household_surve`
ken_maintable$end <- as.numeric(ken_maintable$end)
ken_maintable$end <- as.Date(ken_maintable$end, origin = "1899-12-30")

ken_3_4_1_1_7_1_begin_repeat<-ken_3_4_1_1_7_1_begin_repeat%>%
...
ken_3_4_1_2_1_1_begin_repeat<-ken_3_4_1_2_1_1_begin_repeat%>%
...
ken_3_4_1_1_7_2_begin_repeat<-ken_3_4_1_1_7_2_begin_repeat%>%
  rename("workers"="_3_4_1_1_7_2_calculate",
         "num_seasons"="_3_4_1_1_7_2_1")%>%
  left_join(ken_3_4_1_2_7_2_1_begin_repeat,by=c("kobo_farmer_id","_index","country"))%>%
...
ken_3_4_1_2_1_2_begin_repeat<-ken_3_4_1_2_1_2_begin_repeat%>%
  ...
  mutate(group_workers= case_when(
    ...
    TRUE ~ group_workers))%>%
  left_join(ken_3_4_1_2_1_2_1_begin_repeat,by=c("kobo_farmer_id","_index","country"))%>%
...
ken_3_3_3_2_begin_repeat<-ken_3_3_3_2_begin_repeat%>%
  rename("cropland_practices"="_3_3_3_1_calculate_2",
         "cropland_practices_area"="_3_3_3_2_2")%>%
  filter(!is.na(cropland_practices))%>%
  mutate(cropland_practices_area=as.numeric(cropland_practices_area))%>%
  practices_begin_group(.,.$cropland_practices,.$cropland_practices_area)

ken_3_4_2_2_2_begin_repeat<-ken_3_4_2_2_2_begin_repeat%>%
...
ken_3_3_4_1_3_begin_repeat<- ken_3_3_4_1_3_begin_repeat%>%
  distinct(kobo_farmer_id, "_3_3_4_1_3_2", .keep_all = TRUE) 

##TO CHECK NEED TO UPDATE IN CASE THE DATA WAS UPDATED
ken_post_processing<-read.csv("HOLPA data post-processing_KEN_inputs.csv")%>%
...
area_ken_3_4_3_1_2_begin_repeat<-ken_3_4_3_1_2_begin_repeat%>%
  ...
  mutate(main_crops = str_replace(main_crops, "^(.)", ~str_to_upper(.x)))%>%
  left_join(ken_post_processing,by=c("main_crops","production_unit"))%>%
...
ken_data<- ken_maintable%>%
  left_join(ken_3_4_1_1_7_1_begin_repeat, by=c("kobo_farmer_id"))%>%
  left_join(ken_3_4_1_1_7_2_begin_repeat, by=c("kobo_farmer_id"))%>%
  left_join(ken_3_4_1_2_1_1_begin_repeat, by=c("kobo_farmer_id"))%>%
  left_join(ken_3_4_1_2_1_2_begin_repeat, by=c("kobo_farmer_id"))%>%
  left_join(ken_3_3_3_2_begin_repeat, by=c("kobo_farmer_id"))
  left_join(ken_f_survey, by=c("kobo_farmer_id","country"))%>%
  left_join(ken_3_4_2_2_2_begin_repeat, by=c("kobo_farmer_id"))%>%
  #left_join(ken_soil_results, by=c("kobo_farmer_id"))%>%
  left_join(ken_3_3_4_1_3_begin_repeat, by=c("kobo_farmer_id","country"))%>%
  left_join(area_ken_3_4_3_1_2_begin_repeat, by=c("kobo_farmer_id"))
...
ken_select_multiple_cols <- intersect(colnames(ken_data), unique(select_multiple$name_question))
ken_select_multiple_cols
...
ken_data<-ken_data%>%
  select(!all_of(ken_select_multiple_cols)) 

#ken_data<-ken_data%>%
# left_join(ken_select_multiple, by=c("kobo_farmer_id"))


#####################################
########## RENAME COLUMN NAMES ----
#####################################
# Ensure old column names exist in ken_data
...
existing_cols <- colnames(ken_data)
print(existing_cols)
...
colnames(ken_data) <- ifelse(existing_cols %in% names(rename_vector), rename_vector[existing_cols], existing_cols)
...
colnames(ken_data) <- sapply(colnames(ken_data), rename_slash_columns)


# Check the updated column names
print(colnames(ken_data))
...
sort(unique(ken_data$education_level))
...
ken_data$"soil_fertility_management_ecol_practices/5" <-as.character(ken_data$"soil_fertility_management_ecol_practices/5" )

ken_data$"soil_fertility_management_ecol_practices/5" <- ifelse(ken_data$ecol_practices_mulching_area == "0", "0",
                                                                ifelse(ken_data$ecol_practices_mulching_area != "0", "1", ken_data$"soil_fertility_management_ecol_practices/5"))



### Change name_choice code to numeric codes for: ----
#gender; marital_status
categorical_choices_new_cols <- intersect(colnames(ken_data), unique(ken_global_choices$column_name_new[!is.na(ken_global_choices$name_new)]))
...
for (col in categorical_choices_new_cols) {
  ...
  ken_data[[col]] <- recode(ken_data[[col]], !!!mapping)
}

sort(unique(ken_data$marital_status))
sort(unique(ken_data$gender))
str(ken_data$marital_status)
str(ken_data$gender)
...
names(ken_data)
write.csv(ken_data,"ken_data.csv",row.names=FALSE)
...
names(ken_data)
sort(unique(ken_data$`household_shock/9`))
