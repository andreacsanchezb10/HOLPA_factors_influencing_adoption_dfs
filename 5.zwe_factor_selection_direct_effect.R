...
nzv <- caret::nearZeroVar(zwe_data_analysis, saveMetrics= TRUE) 
nzv # the variables with nzv== TRUE should be remove

nzv_list <- nearZeroVar(zwe_data_analysis)
nzv_list
nzv_factors<- zwe_data_analysis[, nzv_list]
nzv_factors
...
zwe_factors_list <- as.data.frame(colnames(zwe_data_nzvFiltered))%>%
  rename("column_name_new"= "colnames(zwe_data_nzvFiltered)")%>%
  left_join(factors_list_analysis%>%select(column_name_new, category_1),by="column_name_new")%>%
  filter(category_1!="outcome")

zwe_data_nzvFiltered_cor<-create_cor_df(zwe_data_nzvFiltered,zwe_factors_list)
str(zwe_data_nzvFiltered_cor)

plot_correlation_betw_category(zwe_data_nzvFiltered_cor)

##=== STEP 4: REMOVE REDUNDANT FACTORS ======
sort(unique(factors_list_analysis$zimbabwe_remove_adoption_status))
zwe_redundant_list<- intersect(factors_list_analysis$column_name_new[factors_list_analysis$zimbabwe_remove_adoption_status %in%c( "redundant" )],colnames(zwe_data_irrelevantFiltered))
zwe_redundant_list

zwe_data_redundantFiltered<- zwe_data_irrelevantFiltered%>%
  dplyr::select(-all_of(zwe_redundant_list))

dim(zwe_data_redundantFiltered) #200 farmers; 147 variables
names(zwe_data_redundantFiltered)

d<-as.data.frame(c(colnames(zwe_data_redundantFiltered)))%>%
  rename("column_name_new"="c(colnames(zwe_data_redundantFiltered))")%>%
  left_join(factors_list_analysis%>%select(category_1,column_name_new,constructs,constructs_type), by="column_name_new")%>%
...
dim(zwe_data_redundantFiltered)#200 farmers; 1 outcomes, 146 factors retained
#[1] 200 147
...
zwe_factors_list_analysis2 <- as.data.frame(colnames(zwe_data_redundantFiltered))%>%
  rename("column_name_new"= "colnames(zwe_data_redundantFiltered)")%>%
  left_join(factors_list_analysis%>%select(column_name_new, category_1),by="column_name_new")%>%
  filter(category_1!="outcome")

zwe_data_redundantFiltered_cor<-create_cor_df(zwe_data_redundantFiltered,zwe_factors_list_analysis2)
str(zwe_data_redundantFiltered_cor)
...
##=== Run for Mbire and Murehwa (Zimbabwe) ====
zwe_data_numeric <- prepare_numeric_matrix(zwe_data_redundantFiltered)
sft <- run_soft_threshold(zwe_data_numeric, dataset_name = "zwe_data_redundantFiltered")
zwe_picked_power <- 8  # Optionally automate this later
...
zwe_data_adoptionBinary_selectedFactors<- zwe_data_analysis%>%
  select(dfs_adoption_binary,
    all_of(zwe_adoptionBinary_selectedFactors$selected_factors))
...
create_cor_df <- function(data,selected_factors) {
  data_num<-data %>% 
    ...
    select(all_of(zwe_adoptionBinary_selectedFactors$selected_factors))
...
zwe_data_selected_factors_cor<-create_cor_df(zwe_data_redundantFiltered,zwe_adoptionBinary_selectedFactors)
zwe_data_selected_factors_cor
