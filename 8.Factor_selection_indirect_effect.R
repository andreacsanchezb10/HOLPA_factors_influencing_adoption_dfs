...
per_data_analysis<- per_data_analysis%>%
  dplyr::select(-X,crop_type.cacao,-crop_type.camucamu,-crop_type.frutales)%>%
...
########### FACTOR SELECTION ----
#############################################################
feature_selection <- function(factors_list,remove_colum, data_analysis) {
  ##=== STEP 1: REMOVE IRRELEVANT FACTORS ======
  irrelevant_list<- intersect(factors_list$column_name_new[factors_list[[remove_colum]] %in%c("irrelevant")],colnames(data_analysis))
  ...
  redundant_list<- intersect(factors_list$column_name_new[factors_list[[remove_colum]] %in%c( "redundant" )],colnames(data_nzvFiltered))
...
per_data_governance_capacity_redundantFiltered<-feature_selection(factors_list, "peru_remove_governance_capacity",per_data_analysis )
dim(per_data_governance_capacity_redundantFiltered)#200 farmers; 1 outcomes, 147 factors retained

##=== STEP 2: FUZZY FOREST FACTOR SELECTION ======
## Advantages
...
per_data_governance_capacity_numeric <- prepare_numeric_matrix(per_data_governance_capacity_redundantFiltered)
sft_data_governance_capacity <- run_soft_threshold(per_data_governance_capacity_numeric, dataset_name = "per_data_nzvFiltered")
per_data_governance_capacity_picked_power <- 6  # Optionally automate this later

per_governance_capacity <- per_data_governance_capacity_redundantFiltered$governance_capacity
per_governance_capacity
per_governance_capacity_factors <- per_data_governance_capacity_redundantFiltered %>% select(-governance_capacity)


per_governance_capacity_results <- feature_selection_algorithms(per_governance_capacity_factors, per_governance_capacity,
...
plot_accuracy_vs_features(per_crop_type_acc_ff,per_crop_type_acc_rf, per_crop_type_acc_cf,
                          method_name = "A) Peru: Crop type")
#1600*1000
