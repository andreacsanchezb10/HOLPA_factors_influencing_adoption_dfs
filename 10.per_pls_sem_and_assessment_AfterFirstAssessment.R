...
sort(unique(per_measurement_model_afterAssess$constructs))
...
per_constructs_def <- per_measurement_model_afterAssess %>%
  filter(column_name_new %in% intersect(per_measurement_model_afterAssess$column_name_new, colnames(per_data_analysis)))%>%
...
#constructs_def_indirect <- per_measurement_model_afterAssess %>%
#  filter(column_name_new %in% intersect(per_measurement_model_afterAssess$column_name_new, colnames(per_data_analysis_indirect)))%>%
...
per_measurement_model_afterAssess_formula<- build_measurement_model(
  data_analysis=per_data_analysis,
  constructs_variables= per_measurement_model_afterAssess)
per_measurement_model_afterAssess_formula
class(per_measurement_model_afterAssess_formula)
...
per_structural_model_afterAssess_formula<- per_structural_model_afterAssess%>%
  #filter(path=="indirect")%>%
  filter(!is.na(to))%>%
  mutate(formula=paste0("paths(from= '", from, "',to= '", to, "')"))%>%
  select(formula)
per_structural_model_afterAssess_formula

per_structural_model_afterAssess_formula <- map(
  per_structural_model_afterAssess_formula$formula,
  ~ eval(parse_expr(.x)))
per_structural_model_afterAssess_formula

per_structural_model_afterAssess_formula<-do.call(seminr::relationships, c(
...
per_pls_sem_model_complete <- estimate_pls(data = per_data_analysis,
                                           measurement_model = per_measurement_model_afterAssess_formula,
                                           structural_model = per_structural_model_afterAssess_formula)
...
per_measurement_model_afterAssess_formula
per_reflective_constructs <- unique(per_measurement_model_afterAssess_formula$reflective[seq(1, length(per_measurement_model_afterAssess_formula$reflective), 3)])
per_reflective_constructs

per_composite_constructs <- per_measurement_model_afterAssess_formula[names(per_measurement_model_afterAssess_formula) == "composite"]
...
per_reflective_constructs <- unique(per_measurement_model_afterAssess_formula$composite[seq(1, length(per_measurement_model_afterAssess$reflective), 3)])
per_reflective_constructs
...
per_direct_training_participation$training_participation<- as.factor(per_direct_training_participation$training_participation)
